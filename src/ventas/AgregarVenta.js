...
export default function AgregarVenta() {
  ...
  const { fechaVenta, valorVenta } = venta;
  ...
  const onClienteChange = (e) => {
    ...
    setVenta({ ...venta, cliente: { numeroCliente: parseInt(selectedClienteId, 10) } });
  }
  ...
  const onVendedorChange = (e) => {
    ...
    setVenta({ ...venta, vendedor: { numeroVendedor: parseInt(selectedVendedorId, 10) } });
  }

  const onSubmit = async (e) => {
    e.preventDefault();
    const urlBase = "http://localhost:8080/consecionario-app/ventas";
    console.log("Datos de la venta a enviar:", venta);
    try {
      await axios.post(urlBase, venta);
      navegacion('/');
    } catch (error) {
    ...
    <div className='container bg-info'>
      ...
      <div>
        ...
        <div className="row g-3">
          <div className="col">
            <label htmlFor="fechaVenta" className="form-label">Fecha</label>
            <input type="date" className="form-control" id="fechaVenta" name="fechaVenta"
              value={fechaVenta} onChange={(e) => onInputChange(e)} />
          </div>
          <div className="col">
            <label htmlFor="valorVenta" className="form-label">Valor</label>
            <input type="number" step="any" className="form-control" id="valorVenta" name="valorVenta"
              value={valorVenta} onChange={(e) => onInputChange(e)} />
          </div>
        </div>
          <div>
